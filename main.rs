...
impl Atto {
    fn new(filename: Option<String>, preset: &str, vim_mode: bool, command_style: &str) -> Self {
        ...
        parser.set_language(&language).expect("Error loading rust");
    ...
    fn parse_buffer(&mut self) {
        let source_code = self.buffer.join("\n");
        ...
        let paragraph = Paragraph::new(
            self.buffer.iter().enumerate().skip(self.scroll_offset).take(self.terminal_height).map(|(i, line)| {
                ...
                let visible_line = if line_with_number.len() > self.horizontal_scroll_offset {
                    line_with_number[self.horizontal_scroll_offset..].to_string()
                } else {
                    String::new()
                };
                if let Some(tree) = &self.tree {
                    ...
                    for (index, char) in visible_line.char_indices() {
                        cursor.goto_first_child_for_byte(index as usize + self.horizontal_scroll_offset);
                        while cursor.node().kind() != "root" {
                            if cursor.node().byte_range().contains(&index){
                                break;
                            }
                        ...
                        let style = match node.kind() {
                            "function_definition" => Style::default().fg(Color::Cyan),
                            "identifier" => Style::default().fg(Color::Yellow),
                            "keyword" => Style::default().fg(Color::Magenta),
                            "string_literal" => Style::default().fg(Color::Green),
                            "number_literal" => Style::default().fg(Color::Blue),
